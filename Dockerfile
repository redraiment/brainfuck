...
RUN sed -i 's@//.*archive.ubuntu.com@//mirrors.ustc.edu.cn@g' /etc/apt/sources.list \
 && apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends -y flex bison clang-15 lld-15 llvm-15 llvm-15-dev llvm-15-tools zlib1g-dev libtinfo-dev make \
...
RUN sed -i 's/dl-cdn.alpinelinux.org/mirrors.ustc.edu.cn/g' /etc/apk/repositories \
  && apk add --no-cache binutils musl-dev
WORKDIR /workspaces/
